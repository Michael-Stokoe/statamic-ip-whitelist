...
<div class="flex items-center justify-between mb-6">
    ...
    <div class="flex space-x-3">
        ...
        <button id="addIpBtn" class="btn-primary" type="button">
            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="card p-0">
    <div class="p-6 border-b border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-4">
                <button id="addCurrentIpBtn" class="btn btn-sm" type="button">
    ...
    @if(empty($ips))
        <div class="p-12 text-center">
            ...
            <div class="mt-6">
                <button id="addIpBtnEmpty" class="btn-primary" type="button">Add IP Address</button>
            </div>
        </div>
    @else
                    ...
                    @foreach($ips as $ip)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <button onclick="editIp('{{ $ip['ip'] }}', '{{ $ip['name'] }}')" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300 mr-3" type="button">
                                    Edit
                                </button>
                                <button onclick="deleteIp('{{ $ip['ip'] }}')" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300" type="button">
                                    Delete
                                </button>
                            </td>
                        </tr>
    ...
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white dark:bg-neutral-800 rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
            ...
            <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                <button id="submitBtn" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:w-auto sm:text-sm" type="button">Add</button>
                <button onclick="closeModals()" class="w-full inline-flex justify-center rounded-md border border-gray-300 dark:border-gray-600 shadow-sm px-4 py-2 bg-white dark:bg-gray-700 text-base font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:mt-0 sm:w-auto sm:text-sm" type="button">Cancel</button>
            </div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const storeUrl = @json(cp_route('ip-whitelist.store'));
    const updateUrlTemplate = @json(cp_route('ip-whitelist.update', ['ip' => '__IP__']));
    const destroyUrlTemplate = @json(cp_route('ip-whitelist.destroy', ['ip' => '__IP__']));
    ...
    function routeFromTemplate(template, ip) {
        return template.replace('__IP__', encodeURIComponent(ip));
    }
    ...
    window.deleteIp = function(ip) {
        ...
        fetch(routeFromTemplate(destroyUrlTemplate, ip), {
            method: 'DELETE',
            headers: {
            ...
            .then(function(data) {
                ...
                alert(data.message || 'Error removing IP address');
            })
            .catch(function(error) {
                alert(error.message || 'Error removing IP address');
            });
    };

    function submitForm() {
        const url = isEditMode
            ? routeFromTemplate(updateUrlTemplate, originalIp)
            : storeUrl;
        ...
        fetch(url, {
            ...
            .then(function(data) {
                ...
                alert(data.message || 'Error saving IP address');
            })
            .catch(function(error) {
                alert(error.message || 'Error saving IP address');
            });
    }
